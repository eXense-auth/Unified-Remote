...
local exepath = settings.path;

--@help Launch VLC application
actions.launch = function()
--	if OS_WINDOWS then
		libs.server.update({ type = "message", text = "Opening..." });
		pcall(function ()
			os.start("%programfiles(x86)%\\VideoLAN\\VLC\\vlc.exe");
		end);
		pcall(function ()
			os.start("%programfiles%\\VideoLAN\\VLC\\vlc.exe");
		end);
		pcall(function ()
			os.start(exepath);
		end);
...
actions.kill = function ()
	libs.server.update({ type = "message", text = "Closing..." });
	win.kill("vlc.exe");
end
...
function update_status()
	...
	for k,v in pairs(root.children) do
		...
		if (v.name == "rate") then rate = tonumber(v.text); end
		if (v.name == "audiodelay") then audiodelay = tonumber(v.text); end
		if (v.name == "subtitledelay") then subtitledelay = tonumber(v.text); end
	...
	libs.server.update(
		...
		{ id = "rate", progress = rate, },
		{ id = "audiodelay", progress = audiodelay, },
		{ id = "subtitledelay", progress = subtitledelay, },
		{ id = "art", image = image },
		{ id = "play", icon = icon }
	);
...
actions.mrl_upd = function (str)
	mrl = str;
end

actions.play_mrl = function ()
	local str = libs.utf8.trim(mrl);
	if str then
		function send_mrl()
			local url = "http://" .. host .. ":" .. port .. "/requests/status.xml?command=in_play&input=" .. str ;
			return request(url);
		end
		send_mrl ();
		libs.device.toast("Sending...");
	else
		libs.device.toast("Invalid");
	end
end
...
actions.audiodelay_down = function ()
	send("audiodelay", audiodelay - 0.25);
end

actions.audiodelay_up = function ()
	send("audiodelay", audiodelay + 0.25);
end

actions.normal_audiodelay = function ()
	send("audiodelay", "0");
end
...
actions.subtitledelay_down = function ()
	send("subtitledelay", subtitledelay - 0.25);
end

actions.subtitledelay_up = function ()
	send("subtitledelay", subtitledelay + 0.25);
end

actions.normal_subtitledelay = function ()
	send("subtitledelay", "1");
end
